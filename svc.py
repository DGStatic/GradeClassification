...
class DataFrameSelector(sklearn.base.BaseEstimator, sklearn.base.TransformerMixin):
    ...
    def __init__(self, do_predictors=True, do_numerical=True):
        ...
        self.mCategoricalPredictors = ["school", "sex", "address", "famsize", "Pstatus", "Mjob", "Fjob", "reason", "guardian", "schoolsup", "famsup", "paid", "activities", "nursery", "higher", "internet", "romantic"]
        self.mNumericalPredictors = ["age", "Medu", "Fedu", "traveltime", "studytime", "failures", "famrel", "freetime", "goout", "Dalc", "Walc", "health", "absences"]
        self.mLabels = ["G3"]
...
def make_svc_fit_pipeline(my_args):
    ...
    dtargs = {
        "kernel": my_args.kernel,
        "degree": my_args.degree,
        "gamma": my_args.gamma,
        "coef0": my_args.coef0,
        "shrinking": my_args.shrinking,
        "tol": my_args.tol,
        "decision_function_shape": my_args.decision_function_shape,
        "break_ties": my_args.break_ties
    }
    # criterion="entropy", max_depth=2
    items.append(("model", sklearn.svm.SVC(**dtargs)))
...
def make_svc_params_grid_big(my_args):
    ...
    tree_params = {
        ...
        "model__coef0" : [ 0.0, 0.0001],
        "model__shrinking" : [ True, False ],
        "model__tol" : [ 0.01, 0.02, 0.04, 0.08],
        "model__decision_function_shape" : [ "ovr"],
...
def sklearn_metric(y, yhat):
    ...
    print(table)
    print()
    precision = sklearn.metrics.precision_score(y, yhat, zero_division=1)
    recall = sklearn.metrics.recall_score(y, yhat, zero_division=1)
    f1 = sklearn.metrics.f1_score(y, yhat, zero_division=1)
...
def parse_args(argv):
    ...
    parser.add_argument('action', default='SVC',
                        choices=[ "SVC", "score", "show-model", "cross-validate", "grid-search", "show-best-params", "random-search" ],
    ...
    parser.add_argument('--kernel', default="rbf", choices=('linear','poly','rbf','sigmoid','precomputed'),     help=""),
    parser.add_argument('--degree', default=3, type=int, help="only used in poly kernel function"),
    parser.add_argument('--gamma', default="scale", choices=('scale','auto'), help=""),
    parser.add_argument('--coef0', default=0.0, type=float, help="only used in poly and sigmoid kernel function")
    parser.add_argument('--shrinking', default=True, type=bool, help=""),
    parser.add_argument('--tol', default=0.001, type=float, help="" ),
    parser.add_argument('--decision-function-shape', default="ovr", choices=('ovr','ovo'), help="")
    parser.add_argument('--cache-size', default=200, type=int, help="use 500, 1000, or more if you have big RAM")
    parser.add_argument('--break-ties', default=False, type=bool, help="")
