...
def get_data(filename):
    ...
    data = pd.read_csv(filename, dtype={ "school": str })
    return data
...
def do_ann_fit(my_args):
    ...
    model.fit(X, y, epochs=50, verbose=1, callbacks=[early_stopping], validation_split=0.2)
...
def sklearn_metric(y, yhat):
    ...
    print()
    avg = "micro"
    precision = sklearn.metrics.precision_score(y, yhat, average=avg)
    recall = sklearn.metrics.recall_score(y, yhat, average=avg)
    f1 = sklearn.metrics.f1_score(y, yhat, average=avg, zero_division=1)
...
def parse_args(argv):
    ...
    parser.add_argument('action', default='DT',
                        choices=[ "DT", "ANN", "score", "show-model", "cross-validate", "grid-search", "show-best-params", "random-search" ],
